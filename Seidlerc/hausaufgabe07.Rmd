...
```{r}
for(s in c(13,14,15)){
  bestehen <- pnorm(13,mean=mu, sd=s)
  output <- paste("Bei einer Standardabweichung von",s, "werden", bestehen*100, "% mit 'sehr gut' bestehen.")
  print(output)
}
```  
...
*Der Anteil derjeniger, die bestehen ist deutlich höher, als derer, die durchfallen.  Es gibt mehr gute Noten, schlechte Noten sind seltener.*

Warum?

*Der untersuchte Bereich ist größer für schlechte Noten. Ausgehend von einer Normalverteilung würden die Ergebnisse anders ausfallen und im Vergleich zur Mitte weniger Werte aufweisen als diese.*
...
*Die Verteilung ist rechtsschief. Der Gipfel ist auf der linken Seite.*
...
3. Wie viele Studenten bekommen weniger als 10 NP?
    ...
    *`r pnorm(10,mean=mu,sd=3)` Studenten bekommen weniger als 10 Notenpunkte.*
    
4. Wie viele Studenten bekommen weniger als 8 NP?

    *`r pnorm(8,mean=mu,sd=3)` Studenten bekommen weniger als 8 Notenpunkte.*
...
Um in dem besten 1% abzuschließen, muss man zumindest `r qnorm(.99,mean=mu,sd=3)` Notenpunkte bekommen.
...
Der kritische Wert für einen einseitigen $z$-Test ist `r qnorm(p=1-0.06)`. 

Die kritischen Werte für einen zweiseitigen $z$-Test sind $\pm$ `r qnorm(p=1-(0.06/2))`.
...
`r z <- sqrt(7) \left(\frac{10-8}{10}\right)`
...
`r z <- sqrt(50) \left(\frac{10-8}{10}\right)`
...
`r z <- sqrt(20) \left(\frac{7-8}{7}\right)`
...
`r z <- sqrt(25) \left(\frac{7-8}{7}\right)`
