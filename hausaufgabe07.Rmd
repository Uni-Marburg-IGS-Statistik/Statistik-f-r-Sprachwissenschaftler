...
# Die Verteilung mit der standardabweichung "drei" sieht am fairsten aus weil die meisten Leute durchkommen (Die Fläche über den Punktwerten 0-4 scheint am kleinsten zu sein)
...
```{r}
pnorm(13,mean=mu,sd=3)
```
```{r}
for(s in c(3,4,5) ){Überflieger <- pnorm(13,mean=mu,sd=s)
  output <- paste("Bei einer Standabweichung von",s, "gibt es",(1-Überflieger)*100,"% Überflieger.")
  print(output)}
```
...
# Je weniger sehr gute Noten es gibt, desto weniger sehr schlechte Noten gibt es und umgekehrt. Es gibt außerdem mehr schlechte als gute Noten weil der als schlecht definierte Bereich mit 0 bis 4 Punkten größer ist als der als gut definierte Bereich mit 13 bis 15 Punkten.
# bei s=3 gibt es 4,8% Überflieger und 15,9% fallen durch
# bei s=4 gibt es 10,6% Überflieger und 22,7% fallen durch
# bei s=5 gibt es 15,9% Überflieger und 27,4% fallen durch


Warum?

# Weil eine Normalverteilung vom Mittelwert ausgehend nach links und rechts symmetrisch verläuft.
# Folglich gibt es gleich viele Negative und Positive Exreme. Im Fall der Notenpunkte gibt es 
# mehr Leute die Durchfallen als Überflieger weil die Fläche über 0 bis 4 größer ist als jene  über 13 bis 15 Notenpunkten.
...
# Eine Verteilung mit mehr guten Noten, also dem Gipfel auf der rechten Seite, ist linksschief.
...
```{r}
barplot(table(noten.dist$P),beside=TRUE)
```
...
```{r} 
(pnorm(9,mean=mu,sd=3) - pnorm(7,mean=mu,sd=3) ) * 100 Studenten bekommen zwischen 7 und 9 Notenpunkten.
``` 
...
```{r}
pnorm(8,mean=mu,sd=3) + pnorm(7,mean=mu,sd=3,lower.tail=FALSE) 
```
...
Um überdurchschnittlich zu sein, muss man mehr als 
```{r}
qnorm(0.5,mean=mu,sd=3)
``` 
Notenpunkte bekommen. 
...
```{r}
citep("10.1037/0003-066X.44.10.1276")
```

, setzt die Verwaltung das Signikanzniveau auf 0.06. 

irgendwas mit 0,94 und 0,88?
