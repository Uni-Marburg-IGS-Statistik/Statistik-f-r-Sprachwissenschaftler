...
frauen.studiengang.dichte <- ggplot(data=frauen,aes(x=height)) + geom_density(aes(color=major,fill=major), alpha=0.5)
...
kli.hei <- klinisch$height
x <- c(kli.hei)
klinisch.sd <- sqrt(mean((x-mean(x))^2))

lin.hei <- linkk$height
y <- c(lin.hei)
linkk.sd <-  sqrt(mean((y-mean(y))^2))

spe.hei <- speech$height
z <- c(spe.hei)
