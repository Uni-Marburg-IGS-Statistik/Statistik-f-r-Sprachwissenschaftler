...
for (s in c(3, 4, 5)) {
    ...
    output <- paste("Bei einer Standabweichung von", s, "erzielen", ausgezeichnet * 
        100, "% der Studenten eine 1.")
    print(output)
}
```
...
Je größer die Standardabweichung, desto flacher die Kurve und größer die Randbereiche. Das heißt, wenn viele Studenten eine 1 erzielen, fallen auch mehr Studenten durch. Bei einer geringen SD hingegen, liegen die meisten Werte in der Mitte und es gibt sehr wenige schlechte sowie wenige sehr gute Noten.

Warum?

Eine größere Standardabweichung bedeutet, dass auch die Varianz der Daten größer ist und es mehr Extremwerte gibt. Bei einer geringen Standardabweichung hingegen, befinden sich die meisten Werte nahe am Mittelwert. Für unser Beispiel würde das bedeuten, dass ein großer Prozentsatz der Studenten besteht, allerdings mit einer eher mittelmäßigen Note.
...
Die Notenverteilung ist rechtsschief, mit mehr Werten im oberen / rechten Bereich und einem linken Gipfel.
...
Bei diesem Dozenten ist die Verwaltung wirklich unsicher, ob und was für einen Unterschied es geben könnte. (Welche Testart sollte man hier nutzen?) ungerichteter z-Test
...
if (z >= qnorm(0.97))
  {
  output <- paste("Da z=", z,"und |z|>=", x,"ist, wird die H0 abgelehnt.")
  }else{
  output <- paste("Da z=", z,"und |z|<", x,"ist, wird die H0 angenommen.")
  }
print(output)
```
...
Die Verwaltung ist auch auf einen anderen Dozenten aufmerksam geworden, weil manche Studenten behaupten, er würde zu streng benoten. (Welche Testart sollte man hier nutzen?) gerichteter z-Test
