...
# bekommen:
# modus <- code_hier
modus <- names(which.max(tabelle.sorted))
print (modus)
...
# Es kann mehrere Modalwerte geben, wenn Merkmalsausprägungen dieselbe maximale Häufigkeit haben. Wäre in diesem Beispiel die Häufigkeit für (m) und (w) beispielsweise 5, so gäbe es 2 Modalwerte (m und w)
# Der obige Code ist korrekt, da es sich hier um 2 Ausprägungen handelt mit nur einer maximalen Häufigkeit (w)
