...
frauen.studiengang.bw <- ggplot(frauen) + geom_boxplot(aes(x=major,y=height)) 
...
frauen.studiengang.dichte <-  ggplot(frauen) + geom_density(aes(x=height,color=major,fill=major),alpha=0.3) 
...
klinisch.var <-  sum( (klinisch$height - mean(klinisch$height))^2 ) / length(klinisch$height)
klinisch.sd <-sqrt(klinisch.var)
linkk.var <-  sum( (linkk$height - mean(linkk$height))^2 ) / length(linkk$height)
linkk.sd <-sqrt(linkk.var)
speech.var <-  sum( (speech$height - mean(speech$height))^2 ) / length(speech$height)
