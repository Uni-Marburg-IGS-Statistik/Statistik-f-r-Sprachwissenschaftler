...
# Wie sieht es aus mit den Daten zu Kursteilnehmern? Plotten Sie und berechnen
# Sie ein Modell für das Gewicht der Teilnehmer als Funktion von Körpergröße.

ggplot(body,aes(x=body$height,y=body$weight)) +  geom_point() + geom_smooth(method="lm")
m4 <- lm(body$weight ~ body$height, data=body)
print(summary(m4))
