...
rt1 <- rt[rt$subj=="1","RT"]
rt2 <- rt[rt$subj=="2","RT"]
print(var.test(rt1,rt2))

# Sind die Varianzen homogen? Vergessen Sie nicht, dass die Nullhypothese beim
# F-Test "Varianzen Gleich" ist.
# -> aufgrund des F-Tests mit p < 0.05 kann die Nullhypothese verworfen und davon
# ausgegangen werden, dass die Varianzen nicht homogen sind

# Berechenen Sie den Levene Test:
rt.beide <- rt[rt$subj=="1" | rt$subj=="2", c("subj","RT")]
print(leveneTest(rt.beide$RT ~ rt.beide$subj))

# Sind die Varianzen homogen? Vergessen Sie nicht, dass die Nullhypothese beim
# Levene Test "Varianzen Gleich" ist.
# -> beim Levene Test ergibt sich ein F-Wert mit p > 0.05, daher kann davon
# ausgegangen werden, dass die Varianzen homogen sind
...
# Fall?
two.sample <- t.test(rt.beide$RT ~ rt.beide$subj, var.equal = TRUE)
welch <- t.test(rt.beide$RT ~ rt.beide$subj)
...
logrt.plot <- qplot(rt$logRT,geom="density")
print(logrt.plot)
